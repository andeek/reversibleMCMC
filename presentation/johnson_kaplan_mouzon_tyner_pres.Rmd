...
Under Model $k = 2$
  ...
  - $Y_i|\lambda,\kappa \sim$ NegBin($\lambda$, $\kappa$)
...
**Move from Model 1 to Model 2**
  ...
  - Let $x = (1, \theta)$. Set $x' = (2, \theta')$, where
    ...
    $$ 
    \theta' = (\theta_1', \theta_2') = h(\theta, u) = (\theta, \mu e^u)
    $$
...
**Move from Model 2 to Model 1**
  ...
  - Let 
    $$
    (\theta, u) = h'(\theta') = (\theta_1', log(\theta_2'/\mu))
    $$
...
Let $m=\left\{(1,2), (2,1)\right\}$. 
  ...
  $$
  A_m(x,x') = \dfrac{\pi(x')j_m(x')g_m'(u')}{\pi(x)j_m(x)g_m(u)}\left\vert \dfrac{\partial(\theta_k', u')}{\partial (\theta_k, u)}  \right\vert\\
  = \dfrac{\pi(x')}{\pi(x)g(u)}\left\vert \dfrac{\partial(\theta', u')}{\partial (\theta, u)}  \right\vert
  $$
