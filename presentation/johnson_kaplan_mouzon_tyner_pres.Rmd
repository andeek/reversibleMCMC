...
## Finding Appropriate Diagnostics
  ...
  - Diagnostics provide clear and simple rules for determining when a chain is behaving like we expect.
  - But improving efficiency means promoting transitions between models
  - Transitioning between models is not always the favored choice 
     - Chains may "stabilize" quickly inside a model (creating good diagnostics)
     - **Until** the chain moves to sample from a different model (at which point the diagnostics get worse)
  - Recent work focused on the differences in "within run" and "between run" variability
     - Meaning accounting for how much disruption in chain behavior is a result of switching dimensions
...
## Extensions to...
  ...
  - Interacting Sequential MCMC [@jasra2009interacting]
