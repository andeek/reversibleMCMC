...
Under an identically distributed Negative-Binomial model, two parameters $\lambda > 0$, $\kappa > 0$
  ...
  $$
  \mathcal{L}(Y|\lambda, \kappa) = \prod_{i=1}^N \dfrac{\lambda^{Y_i}}{Y_i!}\dfrac{\Gamma \left(1/\kappa + Y_i\right)}{\Gamma \left(1/\kappa\right)\left(1/\kappa + \lambda)^{Y_i}}\left(1+\kappa \lambda\right)^{-1/\kappa}
  $$  
...
Let $m=\left\{(1,2), (2,1)\right\}$. 
  ...
  - Assume probability of moving from 2 to 1 is the same as moving from 1 to 2
...
**Model 1 -> Model 2**
  ...
  \end{array}\right\vert = \begin{array}{cc}
    1 & 0\\
   0 & \mu e^u\\
