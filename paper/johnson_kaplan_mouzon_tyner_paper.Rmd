...
A negative binomial model with parameters $r > 0$ and $p \in (0,1)$ can also be parameterized in terms of its mean $\lambda = r \frac{p}{(1-p)}$ and parameter $\kappa = 1/r$. Then the density can be written as $$ p(Y|\lambda,\kappa) = \frac{\Gamma(1/\kappa + Y)}{Y! \Gamma(1/\kappa)(1/\kappa + \lambda)^{Y}} (1+\kappa \lambda)^{-1/\kappa} $$ 

The variance under this parameterization is $\lambda (1 + \kappa \lambda)$, which gives $\kappa$ the interpretation of representing overdispersion relative to a Poisson distribution with mean $\lambda$.
