...
A simple application of RJMCMC methodologies can be used to determine whether count data is better modeled by a Poisson or negative binomial distribution. This is a simple model selection problem. We present a detailed explanation and implementation of the problem outlined in @green2009reversible.
...
2. For $i = 1,\dots,M$
    ...
    a. If $k^{(i-1)}=1$, attempt a move to model 2. 
        i. If accept $x'$, set $k^{(i)}=2$, $\kappa^{(i-1)} = \theta_{2,2}' = \mu e^u$.
            1. Draw $\kappa^{(i)}$ from $\pi(\kappa | \lambda^{(i-1)}, Y)$ using M-H step, see Appendix \ref{app:MH}
            2. Draw $\lambda^{(i)}$ from $\pi(\lambda | \kappa^{(i)})$ using M-H step, see Appendix \ref{app:MH}
...
The estimated posterior probabilities of the models are $p(k=1) = `r p1`$ and $p(k=2) = `r p2`$. This suggests there is some evidence towards the Negative Binomial model, but the majority favors the Poisson model. The estimated posterior expectation of $\kappa$ under the Negative Binomial model is also very small, at `r mk`. This suggests that the amount of overdispersion in regards to the Poisson distribution is small. Density plots of $\lambda$ and $\kappa$, as well as trace plots used to assess convergence of the sampler are shown in \ref{app:plots}.
...
There are a collection of methods that build off of the RJMCMC ideology by either extending the existing method with other sampling ideas or using the reversible jump within another popular methodology. In this section we briefly discuss three methods - Adaptive RJMCMC [@hastie2005towards], Interacting Sequential MC [@jasra2008interacting], and Simulated Annealing extended for trans-dimensionsal problems [@andrieu2000reversible].
...
\ell(p((2,\theta_{2,1}, \theta_{2,2}) | \mathbf{y}))	 & \propto \log(1-p) + \log(c_\lambda) + (\alpha-1)\log(\lambda) - \beta_{\lambda} \lambda + \log(c_\kappa) + (\alpha-1)\log(\kappa) - \beta_{\kappa} \kappa \\
 ...
 Development of the full conditionals for the two models used for within-model moves in Gibbs sampler: 
...
Consider $\lambda_{(i)}, \kappa^{(i)}$ at the $i^{th}$ state of the chain. We'll use $\text{Gamma}(a_{\lambda}, b_{\lambda})$ and $\text{Gamma}(a_{\kappa}, b_{\kappa})$ as proposal distributions in the Metropolis Hastings steps. To avoid complications with computation of large factorials, we'll construct the M-H steps on the log scale.

1. Draw $\kappa^{(i+1)}$ using indpendence Metropolis Hastings
    ...
    \kappa^{(i)} & \text{ o.w. }\\
    \end{cases}$$
2.  Draw $\lambda^{(i+1)}$ using indpendence Metropolis Hastings
...
$$
  |J| = \left\vert
  \begin{array}{cc}
    \dfrac{\partial \theta_1}{\partial \theta_1} & \dfrac{\partial \theta_1}{\partial \theta_1} \\
...
Then
  $$
  A_{(1,2)}(x,x') = \dfrac{\pi((2,\theta_2'|Y))}{\pi((1,\theta_1)|Y)g(u)}\left\vert \dfrac{\partial(\theta_{2}')}{\partial (\theta_1, u)}  \right\vert\\
...
Then
  ...
  $$
  A_{(2,1)}(x,x') = \dfrac{\pi((1,\theta_1'|Y))g'(u)}{\pi((2,\theta_2)|Y)}\left\vert \dfrac{\partial(\theta_{1}', u')}{\partial (\theta_2)}  \right\vert\\
