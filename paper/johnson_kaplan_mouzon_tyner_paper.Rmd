...
Since the two models are of differing dimensions, reversible jump is necessary to move between the models.
...
2. For $i = 1,\dots,M$
  a. If $k^{(i-1)}=1$, attempt a move to model 2. 
    i. If accept $x'$, set $k^{(i)}=2$, $\kappa^{(i-1)} = \theta_{2,2}' = \mu e^u$.
      ...
      2. Draw $\lambda^{(i)}$ from $\pi(\lambda | \kappa^{(i)})$ using a M-H step, see Appendix \ref{app:MH}
